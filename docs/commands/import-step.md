...
`PCURVE` edges with equal weights on curved B-spline/NURBS surfaces and linear
extrusions of curved directrices retain exact trimmed isocurve geometry and
their source parameter domain.
Cylindrical and
...
extrusions of line, polyline, bounded conic, and B-spline/NURBS directrices
convert to exact tensor-product NURBS surfaces. Revolutions of those directrices
with straight UV iso-trims also
...
simple, separate hole.
Certified curved outer loops can also contain holes inside their convex endpoint
polygon. Other curved multi-loop regions remain unsupported. Periodic
