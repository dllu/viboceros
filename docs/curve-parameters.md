...
including linear source indices for composite sampling. Named span records keep
those indices, parameter bounds, and lengths explicit when mapping leaf spans
into a composite domain. The sibling `tests` module checks this mapping and
keeps analytic references, parameter-scale regressions, and cache-budget checks
separate from production code. Standalone NURBS
