...
and reversed inner walls before planar annulus capping.
...
duplicating marker topology. The application supplies one curve-location pick;
preselection restricts the search, otherwise the document's selectable-object
iterator provides candidates. `src/app/radius.rs` retains failed point prompts.
...
## Viewport and drafting boundaries

The event loop delegates to small modules without putting camera or drafting
state into document transactions:

| Module | Responsibility |
| --- | --- |
| `viewport/camera` | Projection, drafting rays, navigation, and GPU camera coordinates |
| `viewport/extents` | Staged, atomic visible/selected bounds fitting |
...
The `viboceros-drafting` crate separates feature snaps (`object_snap`), plane-local
...
## Geometry and command integration
...
Parallel-view object snapping uses the same relative-coordinate metric for every
...
[spatial query](solid-orientation.md): exact planar-polygon first crossings and
minimum-X regular surface contacts on certified rectangular trims, with explicit
`Unknown` results for unsupported geometry or conflicting shells. Its `planar`
submodule separates exact face extraction, trim winding, and outside-ray tests.
...
not a proof of full Rhino compatibility. The [validation checkpoint](validation-checkpoint.md)
