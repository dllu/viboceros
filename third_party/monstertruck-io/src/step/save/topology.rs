...
where C: StepFormat + StepLength
{
    fn fmt(&self, idx: usize, formatter: &mut Formatter<'_>) -> Result {
        ...
        let kind = if self.seam { "SEAM_CURVE" } else { "SURFACE_CURVE" };
        formatter.write_fmt(format_args!(
            "#{idx} = {kind}('', #{leader_idx}, {associated_geometry}, .CURVE_3D.);\n",
    ...
    S: StepLength,
{
    ...
    fn from_parts(
        ...
        let mut use_count = vec![0; edges.len()];
        ...
        for (face_index, face) in faces.iter().enumerate() {
            for edge in face.boundaries.iter().flatten() {
                use_count[edge.index] += 1;
                match first_face[edge.index] {
                    Some(first) if first == face_index => repeated_on_face[edge.index] = true,
                    None => first_face[edge.index] = Some(face_index),
                    _ => {}
                }
            }
        }
        let seam_edges = use_count
            .into_iter()
            .zip(repeated_on_face)
            .map(|(count, repeated)| count == 2 && repeated)
            .collect::<Vec<_>>();
