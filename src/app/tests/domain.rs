...
fn domain_subcurve_picks_two_locations_for_pre_and_postselection() {
    for preselected in [false, true] {
        ...
        app.command_input = "Domain SubCrv".into();
        app.run_command();
        if !preselected {
            assert!(app.object_prompt.is_some());
            app.select_prompt_objects([id], viboceros_document::SelectionMode::Replace);
        ...
        assert_eq!(
            app.active_command,
            Some(InteractiveCommand::DomainSubCrv { start: None })
        );
        let before = format!("{:?}", app.document);
        assert!(app.accept_drafting_point(point(2., 0., 0.)));
        assert_eq!(
            app.active_command,
            Some(InteractiveCommand::DomainSubCrv {
        ...
        assert!(app.active_command.is_none());
        assert_eq!(app.command_log.back().unwrap(), "Curve domain = [2,8]");
        assert_eq!(format!("{:?}", app.document), before);
    }
}
