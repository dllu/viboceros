...
pub(super) const HELP: &str = "Snap modes: choose Point/End/Mid/Cen/Quad/Near in the toolbar menu. Right-click a mode to isolate/restore it; Shift-click for one pick. At a point prompt type Point, End, Mid, Cen, Quad, Near or NoSnap for one pick. Near and mesh-wire snapping are off by default. Enable mesh Near/Mid with SnapToMeshes Enable or the mesh-wire checkbox. SnapSize changes the active viewport grid snap spacing; ApplyTo=AllViewports changes every view. Grid settings controls grid lines and axes; F7 toggles grid lines. Ortho/F8 constrains picks from the last point; OrthoAngle sets its angular increment. Persistent modes are restored after an accepted point. DisableOsnap/F4 suspends persistent modes without changing the selection.";
