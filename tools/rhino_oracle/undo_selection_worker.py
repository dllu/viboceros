...
state={'busy':False,'done':False,'index':0,'results':[],'owned':set(),'baseline':set()}
def log(s):
...
def finish(error=None):
    ...
    state['owned'].update(o.Id for o in objects() if o.Id not in state['baseline'])
    try: cleanup()
    except Exception as e: error=str(e) if error is None else error+'; '+str(e)
    ...
    d.Modified=False
    try: Rhino.RhinoApp.RunScript('_Exit _No',False)
    ...
    try:
        ...
        if kind in ['DeleteFaces','ExtractMeshFaces']:
            ...
            try:
                ...
                source=d.Objects.AddMesh(mesh)
            finally: mesh.Dispose()
        elif kind=='Explode':
            source=d.Objects.AddPolyline([Rhino.Geometry.Point3d(x,y,0) for x,y in [(0,0),(4,0),(4,3),(0,3),(0,0)]])
        else: source=d.Objects.AddLine(Rhino.Geometry.Point3d(0,0,0),Rhino.Geometry.Point3d(2,0,0))
        other=d.Objects.AddPoint(Rhino.Geometry.Point3d(10,10,0));state['owned'].update([source,other])
        d.Objects.UnselectAll()
        if kind in ['DeleteFaces','ExtractMeshFaces']:
        ...
        ok=bool(Rhino.RhinoApp.RunScript(command,False));records.append(capture(source,other));log('command returned '+str(ok))
        if op['clear']: d.Objects.UnselectAll()
        d.Objects.Select(other)
        records.append(capture(source,other))
        for cmd in ['_Undo','_Redo','_Undo']:
            success=Rhino.RhinoApp.RunScript(cmd,False);records.append(capture(source,other));log(cmd+' '+str(success))
        state['results'].append(dict(id=op['id'],elapsed_ns=0,value=dict(succeeded=ok,states=records)))
