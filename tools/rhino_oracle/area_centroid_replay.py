...
def prepare(request, observed, tight_api=False):
    ...
    for op,row in zip(request["operations"],evidence["results"]):
        ...
        for field in ("constructed_inputs","inputs","insertions","properties","tight_properties"):
        ...
        for field in ("properties","tight_properties"):
            for mass in v[field]:
                if mass is None: continue
                if (not isinstance(mass,dict) or set(mass)!={"area","centroid"} or not finite(mass["area"]) or mass["area"]<0
...
def replay(request,observed,client=None,tight_api=False):
    native,evidence=prepare(request,observed,tight_api)
...
def main():
    ...
    parser.add_argument("--tight-api",action="store_true",help="compare per-object API only, not command markers or selection")
    args=parser.parse_args()
    try:
        report=replay(load_request(args.request),load_request(args.observations),tight_api=args.tight_api)
        output=report.as_dict()
        output["scope"]="explicit-tolerance per-object API only" if args.tight_api else "default per-object API and command marker/selection/attributes; no history-string comparison"
