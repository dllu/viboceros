...
#[test]
fn nonmeeting_curve_fillets_match_saved_rhino_samples() {
    ...
    assert_eq!(actual.results.len(), request.operations.len());
    assert_eq!(
        actual.results.len(),
        ...
        .zip(observed["results"].as_array().unwrap())
    {
        ...
        let epsilon = if row.id.contains("nurbs") {
            1e-7
        } else {
            1e-10
        };
        ...
        assert!((length - expected).abs() < epsilon, "{}: length", row.id);
