...
mod tests {
    #[test]
    fn closed_circle_sweeps_match_owned_rhino_world_space_picks() {
        for (input, observed, count) in [
            (
                include_str!(
            ...
            (
                include_str!(
                    "../../../tools/rhino_oracle/fixtures/sweep1_closed_circle_refit.json"
                ),
                include_str!(
                    "../../../tools/rhino_oracle/observations/sweep1_closed_circle_refit.json"
                ),
                81,
            ),
        ] {
            let request = serde_json::from_str(input).unwrap();
            let observation: serde_json::Value = serde_json::from_str(observed).unwrap();
