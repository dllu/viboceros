...
pub enum GeometryError {
    ...
    #[error(
        "planar NURBS offset could not meet tolerance within fitting budget or model precision"
    )]
    NurbsOffsetFitLimit,
    #[error("NURBS offset requires a planar source curve")]
    NonPlanarCurveOffset,
    ...
    PlaneFitResourceLimit { maximum: usize },
    #[error("best-fit plane coordinates exceed the solver's normalized numerical range")]
    PlaneFitNumericalRange,
    #[error(
        "best-fit plane singular-value decomposition did not resolve a finite full-rank solution"
    )]
    PlaneFitDidNotConverge,
    ...
    #[error(
        "an exact Bezier control cannot be represented by finite Cartesian coordinates and a nonzero finite weight"
    )]
    UnrepresentableBezierControl,
    #[error("a parameter curve leaves the active surface domain")]
    ParameterCurveOutsideSurfaceDomain,
    #[error("bounding box could not meet the requested accuracy within its subdivision budget")]
    BoundingBoxDidNotConverge,
    ...
    PointGridResourceLimit { maximum: usize },
    #[error("edge surface requires two, three, or four open boundary curves")]
    InvalidEdgeSurfaceBoundaries,
    #[error("edge surface exceeds the compatible control limit {maximum}")]
    EdgeSurfaceResourceLimit { maximum: usize },
    #[error("loft needs at least {minimum} sections, got {actual}", minimum = if *.closed { 3 } else { 2 })]
    InsufficientLoftSections { closed: bool, actual: usize },
    #[error("loft {context} exceeds the resource limit {maximum}")]
    LoftResourceLimit {
        context: &'static str,
        maximum: usize,
    },
    #[error("surface grid exceeds the face limit {maximum}")]
    SurfaceGridResourceLimit { maximum: usize },
    #[error("curve parameter intervals must have positive finite length")]
    InvalidCurveParameterInterval,
    ...
    #[error("point-cloud hidden flag count must match point count")]
    InvalidPointCloudHiddenCount,
