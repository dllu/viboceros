...
impl ViewportTabAlignment {
    ...
    pub fn parse(input: &str) -> Option<Self> {
        Self::ALL
            .into_iter()
            .find(|alignment| keyword(input, alignment.label()))
    }
    ...
    Some(
        ...
        } else if name.eq_ignore_ascii_case("ShowEnds") || name.eq_ignore_ascii_case("ShowEndsOff")
        {
            ...
            match args.as_slice() {
                ...
                [option, alignment] if keyword(option, "Align") => {
                    ViewportTabAlignment::parse(alignment)
                        .map(InterfaceCommand::ViewportTabsAlign)
                        .ok_or(InterfaceError::Usage(
                            "ViewportTabs Align=Bottom|Top|Left|Right",
                        ))
                }
                [option]
                    if option
                        .split_once('=')
                        .is_some_and(|(key, _)| keyword(key, "Align")) =>
                {
                    ViewportTabAlignment::parse(option.split_once('=').unwrap().1)
                        .map(InterfaceCommand::ViewportTabsAlign)
                        .ok_or(InterfaceError::Usage(
                            "ViewportTabs Align=Bottom|Top|Left|Right",
                        ))
            ...
            || name.eq_ignore_ascii_case("NewViewport")
            ...
            || name.eq_ignore_ascii_case("CloseViewport")
        {
            if args.is_empty() {
                ...
                } else if name.eq_ignore_ascii_case("NewViewport") {
                    InterfaceCommand::NewViewport
...
impl InterfaceState {
    ...
    pub fn apply(&mut self, command: InterfaceCommand) -> Result<String, InterfaceError> {
        ...
        Ok(match command {
            ...
            InterfaceCommand::ViewportTabsAlign(alignment) => {
